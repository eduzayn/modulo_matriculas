...
// Function to create matricula schema
async function createMatriculaSchema() {
  console.log('Creating matricula schema...');
  
  try {
    // Create schema
    const { data, error } = await supabase.rpc('create_schema', { schema_name: 'matricula' });
    ...
    if (error) {
      ...
      if (userError) {
        console.error('Error creating schema via user metadata:', userError);
        return false;
      }
      
      console.log('Matricula schema created via user metadata');
    } else {
